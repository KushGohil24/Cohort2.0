  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t border-[#e0d6c8]'>
      ...
      <div className='min-w-60'>
        <p onClick={() => setShowFilter(!showFilter)} className='my-2 text-sm flex items-center cursor-pointer gap-2 tracking-[2px] uppercase'>
          Filters
        ...
        <div className={`border border-[#e0d6c8] pl-5 py-3 mt-6 ${showFilter ? '' : 'hidden'} sm:block`}>
          <p className='mb-3 text-xs font-medium tracking-[2px] uppercase text-[#c9a96e]'>Categories</p>
          <div className='flex flex-col gap-2 text-sm text-[#777]'>
            {['Necklaces', 'Rings', 'Earrings', 'Bracelets'].map(cat => (
              <p key={cat} className='flex gap-2 items-center'>
                <input type='checkbox' className='w-3 accent-[#c9a96e]' value={cat} onChange={toggleCategory} /> {cat}
              </p>
            ))}
          </div>
        </div>
        ...
        {/* Type Filter */}
        <div className={`border border-[#e0d6c8] pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          <p className='mb-3 text-xs font-medium tracking-[2px] uppercase text-[#c9a96e]'>Material</p>
          <div className='flex flex-col gap-2 text-sm text-[#777]'>
            {['Gold', 'Silver', 'Diamond', 'Platinum'].map(type => (
              <p key={type} className='flex gap-2 items-center'>
                <input type='checkbox' className='w-3 accent-[#c9a96e]' value={type} onChange={toggleSubCategory} /> {type}
              </p>
            ))}
          </div>
        </div>
